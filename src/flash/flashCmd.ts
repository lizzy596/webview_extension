...
import * as idfConf from "../extConfiguration";
...
export async function verifyCanFlash(
  ...
  if (FlashTask.isFlashing) {
    ...
    // );
    return new Error("One_Task_At_A_Time");
  }
  ...
  if (!(await pathExists(buildPath))) {
    ...
    // );
    return new Error("BUILD_PATH_ACCESS_ERROR");
  }
  ...
  if (!port) {
    ...
    // );
    return new Error("NOT_SELECTED_PORT");
  }
  if (!flashBaudRate) {
    // return Logger.errorNotify(
    ...
    // );
    return new Error("NOT_SELECTED_BAUD_RATE");
  }
  ...
  //     // );
  //     return new Error("NO_DFU_DEVICES_FOUND");
  //   }
  // }
  return continueFlag;
}
